...
class GAN4(object):
    ...
    def BEGAN_dec(self, y,z, hidden_num, act_func, output_channel=3, data_format='NCHW', repeat_num=4):
        ...
        # Decoder0
        x0 = slim.fully_connected(tf.concat([z,y[:,0]],1), np.prod([8, 8, hidden_num]), activation_fn=None)
    ...
    def train_gan(self, **kwargs):
        ...
        with tf.variable_scope("D") as vs_d:
            ...
            d_out = self.BEGAN_dec(z_d, self.y_input, hidden_num=16, act_func=self.act_func, output_channel=self.n_ch_in,
                                   data_format='NCHW', repeat_num=4)
            ...
            for i in tqdm(range(it_per_ep)):
                ...
                result = sess.run(
                    [self.loss_sr, self.loss_sw, self.loss_sf, self.d_loss, self.g_loss, self.measure, self.k_update,
                     self.k_t, self.pulling_term], feed_dict)
                print(result)
